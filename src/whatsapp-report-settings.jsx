      ...
      {!draft?<div className="wrs-loading">{busy?'Loading saved settings…':'Settings could not be loaded. Use Reload to try again.'}</div>:<div className="wrs-layout">
        ...
        <main className="wrs-content" aria-busy={!!busy}>
          ...
          {section==='delivery'&&<>
            ...
            <h4>Quiet hours</h4><div className="wrs-card"><Toggle label="Pause during quiet hours" description="Pause non-OTP messages during this daily IST window, except the important messages ticked below. Missed event messages are not queued; overdue reminders can qualify at a later check." checked={draft.quietHours.enabled} onChange={enabled=>patch('quietHours',{enabled})}/>{draft.quietHours.enabled&&<div className="wrs-form-row wrs-inset"><label>From (IST)<input type="time" value={draft.quietHours.start} onChange={event=>patch('quietHours',{start:event.target.value})}/></label><label>Until (IST)<input type="time" value={draft.quietHours.end} onChange={event=>patch('quietHours',{end:event.target.value})}/></label></div>}{draft.quietHours.enabled&&<details className="wrs-role-picker wrs-inset" open><summary>Still send during quiet hours (important only)<span>{(draft.quietHours.allowedPurposes||[]).length}</span></summary><div>{QUIET_HOUR_PURPOSE_OPTIONS.map(option=><label key={option.key}><input type="checkbox" checked={(draft.quietHours.allowedPurposes||[]).includes(option.key)} onChange={event=>{const current=draft.quietHours.allowedPurposes||[];patch('quietHours',{allowedPurposes:event.target.checked?[...current,option.key]:current.filter(value=>value!==option.key)});}}/>{option.label}</label>)}</div></details>}</div>
