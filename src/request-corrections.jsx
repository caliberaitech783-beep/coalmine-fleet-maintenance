...
function CorrectionCard({record,capabilities,token,Modal,onChanged,fieldOptions={}}){
  ...
  return <article className="correction-card">
    ...
    {(record.canManage||record.canDelete)&&!editing&&!confirmDelete&&<div className="correction-manage">{record.canManage&&<button type="button" className="secondary" disabled={!!working} onClick={()=>{setValues({...record.originalValues,...record.proposedChanges});setReason(record.reason);setError('');setEditing(true)}}><Pencil /> Edit</button>}{record.canDelete&&<button type="button" className="secondary danger" disabled={!!working} onClick={()=>{setError('');setConfirmDelete(true)}}><Trash2 /> Delete</button>}</div>}
