...
export default function DashboardRecordBrowser({ rows, regions, rowsAreScoped = false, title = "Chart records", initialRegion = "", initialSite = "", hideCurrentLocation = false, requestRecords = false, lifecycleRecords = false, ActionsTable, Status, formatDate, RequestTimelineButton = null, timelineToken = "", Dialog = null }) {
  ...
  const columnCount = 9 + (requestRecords ? 2 : 0) + (lifecycleRecords ? 3 : 0) - (showLocationColumn ? 0 : 1);
    ...
    <div id={`${id}-records`} className="dashboard-record-results" role="tabpanel" aria-labelledby={view.selection.region ? `${id}-${view.selection.region}` : undefined}>
      ...
      <div className="dashboard-asset-list" ref={listRef}>
        <ActionsTable key={tableKey} exportTitle={`${title} · ${view.regionLabel}`} printTitle={`${title} · ${view.regionLabel}`}>
          <thead><tr>{requestRecords && <th>Job reference</th>}<th>Status</th><th data-filter-mode={requestRecords ? undefined : "date-sort"}>Started</th><th>Days of breakdown</th><th>Machine / Door no.</th><th>Equipment category</th><th>Equipment group</th><th>Model</th>{showLocationColumn && <th>{requestRecords ? "Request site" : "Current location"}</th>}<th>Serial / chassis no.</th>{requestRecords && <th>Repair category</th>}{lifecycleRecords && <><th>Closed</th><th>MIS verified at</th><th>First trip time</th></>}</tr></thead>
          <tbody>{view.rows.length ? view.rows.map((record, index) => <tr key={record.id || `${record.equipmentName}-${index}`}>
            {requestRecords && <td><b>{record.requestReference}</b></td>}<td data-sort-value={requestStatusSortRank(record.requestStatus)}><Status>{record.requestStatus || "—"}</Status></td><td data-sort-value={sortableDate(record.requestStart)}>{formatDate(record.requestStart)}</td><td data-sort-value={calculateBreakdownMinutes(record.requestStart, record.requestClosed, now)}>{breakdownCell(record)}</td><td>{equipmentMachineLabel(record)}</td><td>{categoryName(equipmentCategoryLabel(record))}</td><td>{equipmentGroupLabel(record)}</td><td>{record.model || "—"}</td>{showLocationColumn && <td>{record.requestSite || record.currentLocation || record.location || record.site || "—"}</td>}<td>{record.manufacturerSerialNo || record.chassisNo || "—"}</td>
