...
export default function DashboardRecordBrowser({ rows, movementDateControl = null, regions, rowsAreScoped = false, title = "Chart records", summaryLabel = "", initialRegion = "", initialSite = "", hideHierarchyFilters = false, showDateFilter = true, showRowNumbers = true, hideCurrentLocation = false, hideSiteColumn = false, hideEquipmentCategory = false, requestRecords = false, lifecycleRecords = false, lifecycleEvent = "", showBdClosingTime = false, onHourlyReport = null, bdBalanceColumns = false, extraColumns = [], ActionsTable, Status, formatDate, RequestTimelineButton = null, timelineToken = "", Dialog = null, Remarks = null }) {
  ...
  const columnCount = 9 + (requestRecords ? 4 : 0) + (showUpdatesColumn ? 1 : 0) + (showClosedColumn ? 1 : 0) + (showVerificationColumns ? 2 : 0) + (showBdClosingTime ? 1 : 0) - (showLocationColumn ? 0 : 1) - (showCategoryColumn ? 0 : 1) + (bdBalanceColumns ? 4 : 0) + extraColumns.length;
  ...
  return <div className="dashboard-record-browser" style={hideHierarchyFilters ? { gridTemplateRows: "minmax(0, 1fr)" } : undefined}>
    ...
    <div id={`${id}-records`} className="dashboard-record-results" role="tabpanel" aria-label={hideHierarchyFilters ? `${title} records` : undefined} aria-labelledby={!hideHierarchyFilters && view.selection.region ? `${id}-${view.selection.region}` : undefined}>
      ...
      <div className="dashboard-asset-list" ref={listRef}>
        <ActionsTable key={tableKey} toolbarTarget={toolbarTarget} toolbarPortal exportTitle={hideHierarchyFilters ? title : `${title} · ${view.regionLabel}`} preserveColumnOrder printTitle={hideHierarchyFilters ? title : `${title} · ${view.regionLabel}`} showRowNumbers={showRowNumbers} disableDateColumnFilter={!showDateFilter} recordDateFilter={showDateFilter ? movementDateControl || { label: "Started", value: recordDateRange, onChange: setRecordDateRange } : false} className={bdBalanceColumns || requestRecords ? "dashboard-location-dates" : undefined} data-verification-last={lifecycleRecords && lifecycleEvent === "mis" ? "true" : undefined}>
