    ...
    <div className={`mine-dashboard ${theme === "dark" ? "mine-dashboard-night" : "mine-dashboard-day"}`}>
      ...
      {assetDrilldown && <Modal className="dashboard-asset-modal" title={`${assetDrilldownTitle} · ${assetDrilldownRows.length}`} close={() => { setAssetDrilldown(""); setAssetDrilldownRegion(""); setAssetDrilldownSite(""); setAssetDrilldownCategory(""); setAssetDrilldownGroup(""); }}><div className="dashboard-asset-drilldown">
        {repairTypeSiteDrilldown ? <>
          <section><h4>Step 1 · Select region</h4><div className="dashboard-asset-summary">{repairTypeRegionBreakdown.length ? repairTypeRegionBreakdown.map((region) => <button type="button" key={region.code} className={assetDrilldownRegion === region.code ? "active" : ""} onClick={() => { setAssetDrilldownRegion(region.code); setAssetDrilldownSite(""); }}><b>{region.count.toLocaleString()}</b>{region.code}</button>) : <p>No regions available</p>}</div></section>
          {selectedRepairTypeRegion && <section><h4><button type="button" className="dashboard-asset-back" aria-label="Back to repair regions" onClick={() => { setAssetDrilldownRegion(""); setAssetDrilldownSite(""); }}><ChevronLeft /> Back</button><span>Step 2 · Select {selectedRepairTypeRegion.code} site</span></h4><div className="dashboard-asset-summary">{repairTypeSiteBreakdown.length ? repairTypeSiteBreakdown.map(({ site, count }) => <button type="button" key={site} className={assetDrilldownSite === site ? "active" : ""} onClick={() => setAssetDrilldownSite(site)}><b>{count.toLocaleString()}</b>{site}</button>) : <p>No sites available</p>}</div></section>}
          {assetDrilldownSite && <section><h4><button type="button" className="dashboard-asset-back" aria-label={`Back to ${selectedRepairTypeRegion.code} repair sites`} onClick={() => setAssetDrilldownSite("")}><ChevronLeft /> Back</button><span>Step 3 · {assetDrilldownSite} request details</span></h4><div className="dashboard-asset-list"><table><thead><tr><th>Job reference</th><th>Equipment name</th><th>Equipment category</th><th>Equipment group</th><th>Make</th><th>Model</th><th>Current location</th><th>Serial / chassis no.</th><th>Status</th><th>Started</th></tr></thead><tbody>{repairTypeSiteRows.length ? repairTypeSiteRows.map((record,index)=><tr key={record.id||`${record.equipmentName}-${index}`}><td><b>{record.requestReference}</b></td><td><b>{record.equipmentName||record.door||"—"}</b></td><td>{equipmentCategoryLabel(record)}</td><td>{equipmentGroupLabel(record)}</td><td>{record.make||"—"}</td><td>{record.model||"—"}</td><td>{record.currentLocation||record.location||"—"}</td><td>{record.manufacturerSerialNo||record.chassisNo||"—"}</td><td><Status>{record.requestStatus}</Status></td><td>{formatTwelveHourDateTime(record.requestStart)}</td></tr>) : <tr><td colSpan="10">No matching requests for {assetDrilldownSite}</td></tr>}</tbody></table></div></section>}
