...
function Dashboard({ goto = () => {}, gotoEquipment = () => {}, gotoBreakdownFleet = () => {}, requests = [], theme = "light", allowedSites = null, allowedRegions = null, restrictToScope = false }) {
  ...
  const [assetDrilldownLevel, setAssetDrilldownLevel] = useState("");
  ...
  return (
    <div className={`mine-dashboard ${theme === "dark" ? "mine-dashboard-night" : "mine-dashboard-day"}`}>
      ...
      {assetDrilldown && <Modal className="dashboard-asset-modal" title={`${assetDrilldownTitle} · ${assetDrilldownRows.length}`} close={() => { setAssetDrilldown(""); setAssetDrilldownLevel(""); setAssetDrilldownRegion(""); setAssetDrilldownSite(""); setAssetDrilldownCategory(""); setAssetDrilldownGroup(""); }}><div className="dashboard-asset-drilldown">
        {siteFirstAssetDrilldown ? <>
          <section><h4>Step 1 · Select location level</h4><div className="dashboard-asset-summary"><button type="button" className={assetDrilldownLevel === "region" ? "active" : ""} onClick={() => { setAssetDrilldownLevel("region"); setAssetDrilldownRegion(""); setAssetDrilldownSite(""); setAssetDrilldownCategory(""); setAssetDrilldownGroup(""); }}><b>{assetDrilldownRegions.length.toLocaleString()}</b>Region</button></div></section>
          {assetDrilldownLevel === "region" && <section><h4>Step 2 · Select region</h4><div className="dashboard-asset-summary">{assetDrilldownRegions.length ? assetDrilldownRegions.map((region) => <button type="button" key={region.code} className={assetDrilldownRegion === region.code ? "active" : ""} onClick={() => { setAssetDrilldownRegion(region.code); setAssetDrilldownSite(""); setAssetDrilldownCategory(""); setAssetDrilldownGroup(""); }}><b>{region.total.toLocaleString()}</b>{region.code}</button>) : <p>No regions available</p>}</div></section>}
          {selectedAssetRegion && <section><h4>Step 3 · Select {selectedAssetRegion.code} site</h4><div className="dashboard-asset-summary">{assetDrilldownSites.length ? assetDrilldownSites.map((site) => <button type="button" key={site.name} className={assetDrilldownSite === site.name ? "active" : ""} onClick={() => { setAssetDrilldownSite(site.name); setAssetDrilldownCategory(""); setAssetDrilldownGroup(""); }}><b>{site.total.toLocaleString()}</b>{site.name}</button>) : <p>No sites available</p>}</div></section>}
          {assetDrilldownSite && <section><h4>Step 4 · {assetDrilldownSite} fleet totals</h4><div className="dashboard-asset-summary">{assetSiteCategoryBreakdown.length ? assetSiteCategoryBreakdown.map(([label, value]) => <button type="button" key={label} className={assetDrilldownCategory === label ? "active" : ""} onClick={() => selectAssetCategory(label)}><b>{value.toLocaleString()}</b>{label}</button>) : <p>No equipment or vehicles found for {assetDrilldownSite}</p>}</div></section>}
          {assetDrilldownCategory && <section><h4>Step 5 · Select {assetDrilldownCategory === "Total vehicles" ? "vehicle" : "equipment"} type</h4><div className="dashboard-asset-groups">{assetSiteGroupBreakdown.length ? assetSiteGroupBreakdown.map(([label, value]) => <button type="button" key={label} className={assetDrilldownGroup === label ? "active" : ""} onClick={() => selectAssetGroup(label)}><span>{label}</span><b>{value.toLocaleString()}</b></button>) : <p>No types found</p>}</div></section>}
          {assetDrilldownGroup && <section><h4>Step 6 · Full details for {assetDrilldownGroup}</h4><div className="dashboard-asset-list"><table><thead><tr><th>Equipment name</th><th>Equipment category</th><th>Equipment group</th><th>Make</th><th>Model</th><th>Current location</th><th>Serial / chassis no.</th></tr></thead><tbody>{assetSiteGroupRows.map((record,index)=><tr key={record.id||`${record.equipmentName}-${index}`}><td><b>{record.equipmentName||record.door||"—"}</b></td><td>{equipmentCategoryLabel(record)}</td><td>{equipmentGroupLabel(record)}</td><td>{record.make||"—"}</td><td>{record.model||"—"}</td><td>{record.currentLocation||record.location||"—"}</td><td>{record.manufacturerSerialNo||record.chassisNo||"—"}</td></tr>)}</tbody></table></div></section>}
        </> : <>
