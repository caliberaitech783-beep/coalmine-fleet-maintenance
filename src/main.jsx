            ...
            <div className="mine-fleet-chart-plot">
              <div className="mine-fleet-chart-regions" style={{ minWidth: `${fleetRegionInsights.reduce((count, region) => count + Math.max(1, region.sites.length), 0) * 108}px` }}><div className="mine-fleet-chart-grid" aria-hidden="true">{fleetChartTicks.map((tick) => <i key={tick} style={{ bottom: `${tick / fleetChartAxisMax * 100}%` }} />)}</div>{fleetRegionInsights.map((region) => <section key={region.code} style={{ flexGrow: Math.max(1, region.sites.length), minWidth: `${Math.max(1, region.sites.length) * 108}px` }} aria-label={`${region.code} fleet sites`}>
                <div className="mine-fleet-chart-sites">{region.sites.map((site) => <div className="mine-fleet-site-entry" key={site.name}>
                  <button type="button" className="mine-fleet-bars-trigger" onClick={(event) => { const segmentColumn = showFleetBreakdowns && event.detail !== 0 ? event.target.closest(".mine-fleet-breakdown-segment")?.closest(".mine-fleet-bar-column") : null; openAssetDrilldown(segmentColumn ? `offroad-site:${site.name}|${segmentColumn.classList.contains("vehicles") ? "vehicles" : "equipment"}` : `${fleetChartMode === "total" ? "site" : "offroad-site"}:${site.name}`); }} aria-label={showFleetBreakdowns ? `${site.name}: ${site.equipment} equipment (${site.breakdown.equipment} breakdown) and ${site.vehicles} vehicles (${site.breakdown.vehicles} breakdown)` : `${site.name}: ${site.equipment} equipment and ${site.vehicles} vehicles`}><FleetSiteBars site={site} axisMax={fleetChartAxisMax} showBreakdown={showFleetBreakdowns} /></button>
