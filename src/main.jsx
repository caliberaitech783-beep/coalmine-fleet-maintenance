...
function Dashboard({ goto = () => {}, gotoEquipment = () => {}, gotoBreakdownFleet = () => {}, requests = [], theme = "light" }) {
  ...
  const initialDrilldownSite = siteScopedSite || movementDrilldownParts[3] || (assetDrilldown.startsWith("trend:") && activeTrendSite !== "all" ? activeTrendSite : "") || (assetDrilldown.startsWith("offroad-site:") ? assetDrilldown.slice(13).split("|")[0] : assetDrilldown.startsWith("site:") ? assetDrilldown.slice(5) : "");
          ...
          {equipmentLoaded?<><div className="mine-fleet-chart-layout" id="fleet-region-plot" aria-label={showFleetBreakdowns ? "Total fleet with breakdowns included at the bottom of each bar" : "Total fleet by region and site"}>
            <div className="mine-fleet-chart-plot">
              <div className="mine-fleet-chart-regions" style={{ minWidth: `${fleetRegionInsights.reduce((count, region) => count + Math.max(1, region.sites.length), 0) * 108}px` }}><div className="mine-fleet-chart-grid" aria-hidden="true">{fleetChartTicks.map((tick) => <i key={tick} style={{ bottom: `${tick / fleetChartAxisMax * 100}%` }} />)}</div>{fleetRegionInsights.map((region) => <section key={region.code} style={{ flexGrow: Math.max(1, region.sites.length), minWidth: `${Math.max(1, region.sites.length) * 108}px` }} aria-label={`${region.code} fleet sites`}>
                <div className="mine-fleet-chart-sites">{region.sites.map((site) => <button type="button" key={site.name} onClick={(event) => { const segmentColumn = showFleetBreakdowns && event.detail !== 0 ? event.target.closest(".mine-fleet-breakdown-segment")?.closest(".mine-fleet-bar-column") : null; openAssetDrilldown(segmentColumn ? `offroad-site:${site.name}|${segmentColumn.classList.contains("vehicles") ? "vehicles" : "equipment"}` : event.detail === 0 || event.target.closest(".mine-fleet-bar, .mine-fleet-site-summary") ? `${fleetChartMode === "total" ? "site" : "offroad-site"}:${site.name}` : fleetChartAllKey); }} aria-label={showFleetBreakdowns ? `${site.name}: ${site.equipment} equipment (${site.breakdown.equipment} breakdown) and ${site.vehicles} vehicles (${site.breakdown.vehicles} breakdown)` : `${site.name}: ${site.equipment} equipment and ${site.vehicles} vehicles`}>
                  <FleetSiteBars site={site} axisMax={fleetChartAxisMax} showBreakdown={showFleetBreakdowns} /><small className="mine-fleet-site-summary"><b>{site.name}</b><span>Total BD {site.breakdown.total.toLocaleString()} · Total Fleet {site.total.toLocaleString()}</span></small>
                </button>)}</div>
