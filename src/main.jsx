...
function MaintenanceForm({ close, normal = false, onSubmit }) {
  ...
  useEffect(() => {
    if (!normal || seconds <= 0) return;
    const timer = setInterval(() => setSeconds((s) => s - 1), 1000);
    return () => clearInterval(timer);
  }, [normal, seconds]);
  useEffect(() => {
    if (normal && seconds <= 0) close();
  }, [normal, seconds, close]);
  const pad = (n) => String(n).padStart(2, "0");
  const systemDate = `${openedAt.getFullYear()}-${pad(openedAt.getMonth() + 1)}-${pad(openedAt.getDate())}`,
    systemTime = `${pad(openedAt.getHours())}:${pad(openedAt.getMinutes())}`,
  ...
  return (
    ...
    >
      <form className="form" onSubmit={submit}>
        {normal && (
          <div
            className={seconds > 0 ? "request-timer" : "request-timer expired"}
          >
            <Clock />
            <span>
              <b>Request form timer</b>
              {seconds > 0
                ? `${seconds} seconds remaining`
                : "Time expired — close and reopen the form"}
            </span>
            <select aria-label="Form time limit" value="120" disabled>
        ...
        <div className="formgrid">
          ...
          <label>
            User name
            <input
              value={normal ? currentEmployeeName : ""}
          ...
          <label>
            Timing
            <input name="time" type="time" defaultValue={systemTime} />
          </label>
          <label>
            Registration number
        ...
        <footer>
          ...
          <button className="primary" disabled={normal && seconds <= 0}>
...
function MasterPage({ name, records = [], onAdd }) {
  const [q, setQ] = useState(""),
    fields = masterFields[name],
        ...
        <table>
          ...
          <tbody>
            {rows.length ? (
              ...
              <tr>
                <td colSpan={fields.length} className="empty-state">
                  No records available
                </td>
              </tr>
            )}
...
function useMasterRecords(name, seed = []) {
  ...
  const add = async (incoming) => {
    ...
    if (!response.ok) throw new Error("Could not save records. Please retry.");
...
Generic = function GenericWithMasters(props) {
  ...
  return masterFields[name] ? (
    <MasterPage name={name} records={records} onAdd={onAdd} />
  ) : (
      ...
      <main>
        ...
        <div className="actioncard">
          ...
          <div>
            ...
            <p>Complete the request within the 120-second form timer.</p>
          </div>
  ...
  return (
    <div className="app">
      ...
      <main className="content">
        ...
        <div className="body">
          {active === "Dashboard" ? (
            <Dashboard goto={setActive} gotoEquipment={gotoEquipment} />
