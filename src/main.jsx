      ...
      <section className="mine-dashboard-grid mine-dashboard-core">
        ...
        <article {...cardAction("event:all", "Request Lifecycle")} className="mine-panel mine-request-lifecycle" aria-label="Opened, closed, verified and idle request trend">
          ...
          {equipmentLoaded?<><div className="mine-request-lifecycle-summary">
            {[{ key: "total", className: "opened", label: "Total Requests", note: "Open + Closed", value: requestLifecycleRows.opened.length + requestLifecycleRows.closed.length }, { key: "closed", label: "Closed", note: "Maintenance completed", value: maintenanceClosedRows.length }, { key: "verified", label: "Verified", note: "MIS verified", value: requestLifecycleRows.verified.length }, { key: "idle", label: "Idle Vehicles", note: "Available, not working", value: requestLifecycleRows.idle.length }, { key: "maintenance", label: "Open in Maint", note: "Active in Maintenance", value: requestLifecycleAvailability.maintenance }, { key: "mis", label: "Open in MIS", note: "Closed - Verified", value: requestLifecycleAvailability.mis }].map((item) => <button type="button" key={item.key} className={item.className || item.key} onClick={() => item.key === "total" ? openAssetDrilldown("event:all") : item.key === "maintenance" ? openAssetDrilldown("event:opened") : item.key === "mis" ? openAssetDrilldown("event:closed") : openAssetDrilldown(`event:${item.key}`)}><i /><span><b>{item.label}</b><small>{item.note}</small></span><strong>{item.value.toLocaleString()}</strong></button>)}
          </div>
