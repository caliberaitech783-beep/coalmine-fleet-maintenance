...
function Dashboard({ goto = () => {}, gotoEquipment = () => {}, gotoBreakdownFleet = () => {}, requests = [], theme = "light" }) {
  ...
  const fleetBreakdownScaleMax = Math.max(5, ...fleetRegionInsights.flatMap((region) => region.sites.flatMap((site) => [site.breakdown.equipment, site.breakdown.vehicles])));
  const fleetChartTicks = [...new Set([...Array.from({ length: fleetChartAxisMax / fleetChartStep + 1 }, (_, index) => index * fleetChartStep), ...(showFleetBreakdowns ? [fleetBreakdownScaleMax] : [])])].sort((a, b) => a - b);
    ...
    <div className={`mine-dashboard ${theme === "dark" ? "mine-dashboard-night" : "mine-dashboard-day"}${showFleetBreakdowns ? " breakdown-dashboard-view" : ""}`}>
      ...
      <section className="mine-dashboard-feature-row" aria-label="Fleet and repair overview">
        <article {...cardAction(fleetChartAllKey, showFleetBreakdowns ? "Breakdown fleet" : "Total Fleet")} className={`mine-panel mine-fleet-region-chart${showFleetWatermark ? " watermarked" : ""}`} data-mode={fleetChartMode} aria-label={`${showFleetBreakdowns ? "Fleet with breakdowns" : "Total fleet"} by region and site graph`}>
          ...
          {equipmentLoaded?<>{showFleetBreakdowns && <p className="mine-fleet-scale-note" id="fleet-scale-note">Breakdown scale: 0–{fleetBreakdownScaleMax} enlarged uniformly; totals continue above this range.</p>}<div className="mine-fleet-chart-layout" id="fleet-region-plot" aria-describedby={showFleetBreakdowns ? "fleet-scale-note" : undefined} aria-label={showFleetBreakdowns ? "Total fleet with breakdowns included at the bottom of each bar" : "Total fleet by region and site"}>
            <div className="mine-fleet-chart-plot">
              <div className="mine-fleet-chart-regions" style={{ minWidth: `${fleetRegionInsights.reduce((count, region) => count + Math.max(1, region.sites.length), 0) * 108}px` }}><div className="mine-fleet-chart-grid" aria-hidden="true">{fleetChartTicks.map((tick) => <i key={tick} style={{ bottom: `${fleetBarHeightPercent(tick, fleetChartAxisMax, showFleetBreakdowns, fleetBreakdownScaleMax)}%` }} />)}</div>{fleetRegionInsights.map((region) => <section key={region.code} style={{ flexGrow: Math.max(1, region.sites.length), minWidth: `${Math.max(1, region.sites.length) * 108}px` }} aria-label={`${region.code} fleet sites`}>
                <div className="mine-fleet-chart-sites">{region.sites.map((site) => <div className="mine-fleet-site-entry" key={site.name}>
                  ...
                  }} aria-label={showFleetBreakdowns ? `${site.name}: ${site.equipment} equipment (${site.breakdown.equipment} breakdown) and ${site.vehicles} vehicles (${site.breakdown.vehicles} breakdown)` : `${site.name}: ${site.equipment} equipment and ${site.vehicles} vehicles`}><FleetSiteBars site={site} axisMax={fleetChartAxisMax} showBreakdown={showFleetBreakdowns} breakdownScaleMax={fleetBreakdownScaleMax} /></button>
                  <small className="mine-fleet-site-summary"><b>{site.name}</b><button type="button" onClick={() => openAssetDrilldown(`offroad-site:${site.name}`)}>Total BD {site.breakdown.total.toLocaleString()}</button><button type="button" onClick={() => openAssetDrilldown(`site:${site.name}`)}>Total Fleet {site.total.toLocaleString()}</button></small>
                </div>)}</div>
