...
import { buildOemBreakdownRows, buildOemBreakdownChart, selectOemBreakdownRows } from "./oem-breakdown-model.mjs";
...
function Dashboard({ goto = () => {}, gotoEquipment = () => {}, gotoBreakdownFleet = () => {}, requests = [], requestsError = "", requestsUpdatedAt = 0, onRefreshRequests, theme = "light" }) {
  ...
  const oemDetailRows = oemDrilldown ? selectOemBreakdownRows(oemChart.rows, oemDrilldown) : [];
  const oemDetailRequests = oemDetailRows.flatMap((row) => row.requests);
  const oemSelectedLabel = oemChart.oems.find((oem) => oem.key === (oemDrilldown?.oem || dashboardOem))?.label || "All OEMs";
  ...
  return (
    <div className={`mine-dashboard ${theme === "dark" ? "mine-dashboard-night" : "mine-dashboard-day"}${showFleetBreakdowns ? " breakdown-dashboard-view" : ""}${showOemBreakdowns ? " mine-oem-view" : ""}`}>
      ...
      {oemDrilldown && <Modal className="dashboard-asset-modal" title={`OEM BD · ${oemSelectedLabel} · ${oemDrilldown.site || (dashboardSite !== "all" ? dashboardSite : "All selected sites")} · ${oemDetailRows.length} assets`} close={() => setOemDrilldown(null)}>
