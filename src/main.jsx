    ...
    setTicketState((current) => {
      ...
      const next = category && ticket.category !== category
        ...
        : records.some((row) => row.reference === ticket.reference)
          ? records.map((row) => row.reference === ticket.reference ? {creatorPhone: row.creatorPhone, creatorPhoneHref: row.creatorPhoneHref, ...ticket} : row)
          : [ticket, ...records];
