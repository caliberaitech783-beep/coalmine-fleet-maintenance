...
function Dashboard({ goto = () => {}, gotoEquipment = () => {}, gotoBreakdownFleet = () => {}, requests = [], theme = "light", allowedSites = null, allowedRegions = null, restrictToScope = false }) {
  ...
  const assetDrilldownVisibleRows = assetDrilldownGroup ? assetDrilldownRows.filter((record) => equipmentGroupLabel(record) === assetDrilldownGroup) : assetDrilldownRows;
  const assetCategoryBreakdown = summarizeEquipment(assetDrilldownRows, equipmentCategoryLabel);
  const assetGroupBreakdown = summarizeEquipment(assetDrilldownRows);
  const assetDrilldownTitle = assetDrilldown === "equipment" ? "Total equipment" : assetDrilldown === "vehicle" ? "Total vehicles" : assetDrilldown === "onroad" ? "On road equipment" : assetDrilldown === "offroad" ? "Off road equipment" : assetDrilldown === "idle" ? "Idle equipment" : assetDrilldown === "unknown" ? "Status not set" : assetDrilldown.startsWith("region:") ? `${assetDrilldown.slice(7)} equipment` : assetDrilldown.startsWith("group:") ? assetDrilldown.slice(6) : "Total equipment and vehicles";
  const openAssetDrilldown = (key, group = "") => {
    setAssetDrilldown(key);
    setAssetDrilldownGroup(group);
  };
  const assetTooltip = (records = []) => {
    const categoryRows = summarizeEquipment(records, equipmentCategoryLabel);
    const groupRows = summarizeEquipment(records).slice(0, 6);
    return <div className="dashboard-kpi-tooltip dashboard-asset-tooltip"><b>Drilldown</b>{categoryRows.map(([label, value]) => <i key={label}>{label}: {value}</i>)}<b>Equipment group</b>{groupRows.length ? groupRows.map(([label, value]) => <i key={label}>{label}: {value}</i>) : <i>No equipment</i>}</div>;
  };
  ...
  return (
    <div className={`mine-dashboard ${theme === "dark" ? "mine-dashboard-night" : "mine-dashboard-day"}`}>
      ...
      <section className="mine-overview-charts" aria-label="Fleet overview charts">
        <article className="mine-overview-chart">
          ...
          <div className="mine-overview-chart-body">
            <button type="button" className="mine-overview-donut" aria-label={`${assetCounts.total} total assets`} onClick={() => openAssetDrilldown("all")} style={{ background: `conic-gradient(#4f86c6 0 ${equipmentShare}%, #72c99e ${equipmentShare}% 100%)` }}><span><strong>{assetCounts.total.toLocaleString()}</strong><small>All total</small></span>{assetTooltip(visibleEquipment)}</button>
            <div className="mine-overview-legend">
              <button type="button" onClick={() => openAssetDrilldown("equipment")}><i className="mine-chart-blue" /><span>Total equipment<small>Non-vehicle equipment</small></span><strong>{assetCounts.equipment.toLocaleString()}</strong>{assetTooltip(rowsForAssetDrilldown("equipment"))}</button>
              <button type="button" onClick={() => openAssetDrilldown("vehicle")}><i className="mine-chart-green" /><span>Total vehicles<small>Vehicles only</small></span><strong>{assetCounts.vehicles.toLocaleString()}</strong>{assetTooltip(rowsForAssetDrilldown("vehicle"))}</button>
            </div>
          </div>
        </article>
        <article className="mine-overview-chart">
          ...
          <div className="mine-overview-chart-body">
            <button type="button" className="mine-overview-donut" aria-label={`${kpis.availability}% on road`} onClick={() => openAssetDrilldown("onroad")} style={{ background: `conic-gradient(#72c99e 0 ${onRoadShare}%, #df776e ${onRoadShare}% ${onRoadShare + offRoadShare}%, #e5aa55 ${onRoadShare + offRoadShare}% ${onRoadShare + offRoadShare + idleShare}%, #d9e3e7 ${onRoadShare + offRoadShare + idleShare}% 100%)` }}><span><strong>{kpis.availability}%</strong><small>On road</small></span>{assetTooltip(rowsForAssetDrilldown("onroad"))}</button>
            <div className="mine-overview-legend">
              <button type="button" onClick={() => openAssetDrilldown("onroad")}><i className="mine-chart-green" /><span>On road<small>Available for operation</small></span><strong>{kpis.onRoad.toLocaleString()}</strong>{assetTooltip(rowsForAssetDrilldown("onroad"))}</button>
              <button type="button" onClick={() => openAssetDrilldown("offroad")}><i className="mine-chart-red" /><span>Off road<small>Maintenance or breakdown</small></span><strong>{kpis.offRoad.toLocaleString()}</strong>{assetTooltip(rowsForAssetDrilldown("offroad"))}</button>
              <button type="button" onClick={() => openAssetDrilldown("idle")}><i className="mine-chart-idle" /><span>Idle<small>Operational, currently not working</small></span><strong>{kpis.idle.toLocaleString()}</strong>{assetTooltip(rowsForAssetDrilldown("idle"))}</button>
              {kpis.unknown > 0 && <button type="button" onClick={() => openAssetDrilldown("unknown")}><i className="mine-chart-grey" /><span>Status not set<small>Not counted as off road</small></span><strong>{kpis.unknown.toLocaleString()}</strong>{assetTooltip(rowsForAssetDrilldown("unknown"))}</button>}
            </div>
          </div>
        </article>
      </section>
      ...
      <section className="mine-dashboard-grid">
        ...
        <article className="mine-panel"><header><div><span className="mine-eyebrow">Equipment by region</span><h2>Operating regions</h2><p>Registered assets across sites</p></div><button type="button" onClick={() => goto("Region master")}><ChevronRight /></button></header><div className="mine-region-bars">{regionBars.map((region) => <button type="button" key={region.code} onClick={() => openAssetDrilldown(`region:${region.code}`)}><span>{region.code}</span><div><i style={{ width: `${(region.total / maxRegionTotal) * 100}%` }} /></div><b>{region.total.toLocaleString()}</b>{assetTooltip(rowsForAssetDrilldown(`region:${region.code}`))}</button>)}</div></article>
        <article className="mine-panel"><header><div><span className="mine-eyebrow">Fleet composition</span><h2>Vehicle by group</h2><p>Most represented equipment categories</p></div></header><div className="mine-type-bars">{vehicleTypes.length ? vehicleTypes.map(([label, value]) => <button type="button" key={label} onClick={() => openAssetDrilldown(`group:${label}`)}><span>{label}</span><div><i style={{ width: `${(value / Math.max(1, vehicleTypes[0][1])) * 100}%` }} /></div><b>{value.toLocaleString()}</b>{assetTooltip(rowsForAssetDrilldown(`group:${label}`))}</button>) : <p className="mine-empty">No equipment records yet</p>}</div></article>
      ...
      {assetDrilldown && <Modal className="dashboard-asset-modal" title={`${assetDrilldownTitle} · ${assetDrilldownRows.length}`} close={() => { setAssetDrilldown(""); setAssetDrilldownGroup(""); }}><div className="dashboard-asset-drilldown"><section><h4>Step 1 · KPI summary</h4><div className="dashboard-asset-summary"><span><b>{assetDrilldownRows.length.toLocaleString()}</b>Total records</span>{assetCategoryBreakdown.map(([label, value]) => <button type="button" key={label} onClick={() => openAssetDrilldown(label === "Total vehicles" ? "vehicle" : label === "Total equipment" ? "equipment" : "all")}><b>{value.toLocaleString()}</b>{label}</button>)}</div></section><section><h4>Step 2 · Equipment category / group</h4><div className="dashboard-asset-groups">{assetGroupBreakdown.length ? assetGroupBreakdown.map(([label, value]) => <button type="button" key={label} className={assetDrilldownGroup === label ? "active" : ""} onClick={() => setAssetDrilldownGroup(assetDrilldownGroup === label ? "" : label)}><span>{label}</span><b>{value.toLocaleString()}</b></button>) : <p>No equipment group found</p>}</div></section><section><h4>Step 3 · Equipment list{assetDrilldownGroup ? ` · ${assetDrilldownGroup}` : ""}</h4><div className="dashboard-asset-list"><table><thead><tr><th>Equipment name</th><th>Equipment category</th><th>Equipment group</th><th>Make</th><th>Model</th><th>Current location</th><th>Serial / chassis no.</th></tr></thead><tbody>{assetDrilldownVisibleRows.map((record,index)=><tr key={record.id||`${record.equipmentName}-${index}`}><td><b>{record.equipmentName||record.door||"—"}</b></td><td>{equipmentCategoryLabel(record)}</td><td>{equipmentGroupLabel(record)}</td><td>{record.make||"—"}</td><td>{record.model||"—"}</td><td>{record.currentLocation||record.location||"—"}</td><td>{record.manufacturerSerialNo||record.chassisNo||"—"}</td></tr>)}</tbody></table></div></section></div></Modal>}
...
function BreakdownTable({ rows = breakdowns, showBreakdownDays = false, stickyHeader = false, showAudio = false, showTurnaroundTime = false, showReason = false, showCreatedBy = false, showMakeModel = false, onApproveIdeal, stableToolbar = false }) {
