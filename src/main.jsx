...
function Dashboard({ goto = () => {}, gotoEquipment = () => {}, gotoBreakdownFleet = () => {}, requests = [], theme = "light", allowedSites = null, allowedRegions = null, restrictToScope = false }) {
  ...
  const assetDrilldownSites = [...new Set(fleetRegionInsights.flatMap((region) => region.sites.map((site) => site.name)))];
  const assetSiteBreakdown = assetDrilldownSites.map((site) => [site, assetDrilldownRows.filter((record) => recordBelongsToSite(record, site)).length]);
  ...
  return (
    <div className={`mine-dashboard ${theme === "dark" ? "mine-dashboard-night" : "mine-dashboard-day"}`}>
      ...
      {assetDrilldown && <Modal className="dashboard-asset-modal" title={`${assetDrilldownTitle} · ${assetDrilldownRows.length}`} close={() => { setAssetDrilldown(""); setAssetDrilldownSite(""); setAssetDrilldownCategory(""); setAssetDrilldownGroup(""); }}><div className="dashboard-asset-drilldown">
        {siteFirstAssetDrilldown ? <>
          <section><h4>Step 1 · Select site</h4><div className="dashboard-asset-summary">{assetSiteBreakdown.length ? assetSiteBreakdown.map(([site, value]) => <button type="button" key={site} className={assetDrilldownSite === site ? "active" : ""} onClick={() => setAssetDrilldownSite(site)}><b>{value.toLocaleString()}</b>{site}</button>) : <p>No sites available</p>}</div></section>
          {assetDrilldownSite && <section><h4>Step 2 · {assetDrilldownSite} equipment and vehicles</h4><div className="dashboard-asset-list"><table><thead><tr><th>Equipment name</th><th>Equipment category</th><th>Equipment group</th><th>Make</th><th>Model</th><th>Current location</th><th>Serial / chassis no.</th></tr></thead><tbody>{assetSiteRows.length ? assetSiteRows.map((record,index)=><tr key={record.id||`${record.equipmentName}-${index}`}><td><b>{record.equipmentName||record.door||"—"}</b></td><td>{equipmentCategoryLabel(record)}</td><td>{equipmentGroupLabel(record)}</td><td>{record.make||"—"}</td><td>{record.model||"—"}</td><td>{record.currentLocation||record.location||"—"}</td><td>{record.manufacturerSerialNo||record.chassisNo||"—"}</td></tr>) : <tr><td colSpan="7" className="empty-state">No equipment or vehicles found for {assetDrilldownSite}</td></tr>}</tbody></table></div></section>}
        </> : <>
