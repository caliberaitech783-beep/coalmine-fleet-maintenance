...
export default function OemBreakdownDetails({ selection, title, MaintenanceRemarks, ...tableProps }) {
  ...
  return <div className={`mine-oem-details${filtersHidden ? " filters-hidden" : ""}`}>
    ...
    <div className="mine-oem-site-tables" role="region" tabIndex={0} aria-label="Site-wise fleet records">
      <DashboardRecordBrowser {...tableProps} summaryTarget={summaryTarget} rows={groupedRows} regions={selection.regions} rowsAreScoped title={title} groupBySite hideHierarchyFilters showDateFilter={false} showRowNumbers requestRecords={!selection.fleetOnly} extraColumns={selection.fleetOnly ? [extraColumns[0]] : columns} />
    </div>
  </div>;
}
