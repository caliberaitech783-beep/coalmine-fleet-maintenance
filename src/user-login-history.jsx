...
export function UserLoginHistory({token,row,Modal,Table,formatDate,deviceDetails,onClose}){
  ...
  return <Modal title={<span className="login-history-heading"><span>{row.name||row.login} · Login sessions · Last 24 hours</span><span className="login-history-total" role="status">Total time spent (last 24 hours): <strong>{loading?'Loading…':error?'Unavailable':formatSessionDuration(timing.total)}</strong></span></span>} close={onClose}>
