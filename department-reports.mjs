...
report('mis', DEPARTMENT_REPORT_TITLES[3], 'MIS verification more than 30 minutes after first trip. Difference = verified date/time minus first-trip date/time.', [...base,closed,col('firstTripAt','First Trip Made',firstTrip),col('verifiedAt','MIS Verified Date & Time'),col('difference','Difference',r => duration(firstTrip(r),r.verifiedAt)),col('mismatch','Mismatch',r => indiaDateTimeEpoch(r.verifiedAt)-indiaDateTimeEpoch(firstTrip(r))>1800000 ? 'Delay' : ''),col('verifiedBy','MIS user'),col('driverName','Driver Name'),site,ref],requests.filter(r => indiaDateTimeEpoch(r.verifiedAt)-indiaDateTimeEpoch(firstTrip(r))>1800000), r => r.verifiedAt),
