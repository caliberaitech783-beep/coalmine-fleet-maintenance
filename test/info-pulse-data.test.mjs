...
test('production first-trip pending queue includes only requests generated on rollout date onward and excludes completed production entries', () => {
  const records = [
  ...
  assert.deepEqual(rows.map(row => row.key), ['today-pending']);
