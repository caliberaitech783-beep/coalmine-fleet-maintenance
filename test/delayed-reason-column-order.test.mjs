...
test('maintenance workflow tables lead with door, status, actions and the request timing, then the rest', () => {
  ...
  assert.deepEqual(ordered.map(column => column.label), [
    'Door no.', 'Status', 'Actions', 'Started', 'Days of breakdown', 'Reason', 'Breakdown type', 'Equipment group', 'Make', 'Model', 'Daily remarks', 'Delayed reason',
    'Site location', 'Idle reason', 'Job reference',
  ]);
...
test('MIS tables use the same leading layout with their Production date and time column', () => {
  ...
  assert.deepEqual(requestColumnsInWorkflowOrder(columns, true).map(column => column.label), ['Door no.', 'Status', 'Actions', 'Production date and time', 'Days of breakdown', 'Closed time', 'Job reference']);
});
