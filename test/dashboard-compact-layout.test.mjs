...
test("site breakdown view reconciles one-line site totals and opens day-wise controls", () => {
  ...
  assert.match(client, /<span>Site name<\/span><span>BD Open<\/span><span>BD In<\/span><span>BD Out<\/span><span>BD Balance<\/span><span>Availability count impact<small>\{availabilityDateLabel\}<\/small><\/span>/);
