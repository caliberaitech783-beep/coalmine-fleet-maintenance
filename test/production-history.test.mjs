...
test("exclusion is applied only to Production User history, not other queues", () => {
  ...
  assert.match(source, /historyRows=isMis\?closedRequests\.filter\(\(row\)=>Boolean\(row\.verifiedAt\)\)\.filter\(visibleInMisHistory\):isProduction\?closedRequests\.filter\(visibleInProductionHistory\):isMaintenance\?closedRequests\.filter\(visibleInMaintenanceHistory\):closedRequests/);
