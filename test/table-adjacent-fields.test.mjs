...
test('machine and meter columns stay beside status and breakdown reason with original data indices', () => {
  const labels = ['Status', 'Days of breakdown', 'Current location', 'Started', 'Machine / Door no.', 'Equipment category', 'Type of breakdown', 'Reason of breakdown', 'Equipment group', 'Model', 'HMR', 'KMR'];
  const columns = labels.map((label, index) => ({label, index, key: String(index)}));
  const ordered = jobReferenceColumnsLast(columns);
  assert.deepEqual(ordered.map(c => c.label), ['Status', 'Machine / Door no.', 'Days of breakdown', 'Current location', 'Started', 'Equipment category', 'Type of breakdown', 'Reason of breakdown', 'HMR', 'KMR', 'Equipment group', 'Model']);
  assert.deepEqual(ordered.map(c => c.index), [0, 4, 1, 2, 3, 5, 6, 7, 10, 11, 8, 9]);
