...
test('XLSX bundle has one ordered worksheet per selected report with complete relationships',()=>{
  ...
  tables.forEach((table,index)=>{
    ...
    assert.equal([...xml.matchAll(/<row r=/g)].length,table.rows.length+1);
    const widths=[...xml.matchAll(/<col min="(\d+)" max="(\d+)" width="([\d.]+)" customWidth="1"\/>/g)];
    assert.equal(widths.length,table.columns.length);
    widths.forEach((match)=>{assert.equal(match[1],match[2]);assert.ok(Number(match[3])>=12&&Number(match[3])<=48);});
  });
  assert.deepEqual(cells(files.get('xl/worksheets/sheet1.xml')).map((cell)=>cell.value),['User','Activity at','Operator A','5:00 PM 15-09-2026']);
...
test('XLSX bundle retains empty selected tables and supplies a valid worksheet when none are selected',()=>{
  ...
  assert.deepEqual(cells(files.get('xl/worksheets/sheet1.xml')),[{reference:'A1',value:'User'}]);
  assert.deepEqual(cells(files.get('xl/worksheets/sheet2.xml')),[{reference:'A1',value:'Reference'}]);
  assert.match(files.get('xl/worksheets/sheet3.xml'),/<sheetData><row r="1"><\/row><\/sheetData>/);
...
test('XLSX keeps all long rows, Unicode, XML characters and whitespace without interpreting formulas',()=>{
  ...
  assert.equal([...xml.matchAll(/<row r=/g)].length,rows.length+1);
  const values=cells(xml).map((cell)=>cell.value);
  assert.ok(values.includes(long));
  for(let index=0;index<5005;index++)assert.equal(values[(index+1)*2],rows[index][0]);
...
test('wide bundle worksheets use valid references beyond Z and legacy workbooks keep the Report tab',()=>{
  ...
  assert.deepEqual(cells(bundle.get('xl/worksheets/sheet1.xml')).at(-1),{reference:'AB2',value:'VALUE_27'});
  const legacy=storedFiles(buildXlsxWorkbookBuffer('Legacy title',columns,rows));
  assert.deepEqual(sheetNames(legacy),['Report']);
  assert.deepEqual(cells(legacy.get('xl/worksheets/sheet1.xml')),cells(bundle.get('xl/worksheets/sheet1.xml')));
  ...
  assert.deepEqual(cells(empty.get('xl/worksheets/sheet1.xml')),[{reference:'A1',value:'Reference'}]);
});
