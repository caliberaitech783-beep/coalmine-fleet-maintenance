...
test("the closing time column moves directly after Started whatever the two columns are called", () => {
...
test("the breakdown table renders, exports and sorts a closing time column placed after Started", () => {
  ...
  assert.match(breakdown, /\["start", "Started"\], \.\.\.\(showClosedAt \? \[\["closedAt", closedAtLabel\]\] : \[\]\), \["hours",/);
  assert.match(breakdown, /if \(key === "closedAt"\) return formatTwelveHourDateTime\(row\.closedAt\);/);
  assert.match(breakdown, /<td>\{formatTwelveHourDateTime\(r\.start\)\}<\/td>\r?\n\s*\{showClosedAt && <td>\{formatTwelveHourDateTime\(r\.closedAt\)\}<\/td>\}/);
