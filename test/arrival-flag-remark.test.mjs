...
function harness({row=eligible,user={site:'Sasti OB'},beforeUpdate}={}){
  ...
  const context={
    ...
    pool:{async query(sql,values){
      ...
      assert.match(sql,/SET arrival_flagged_at=NOW\(\),arrival_flagged_by=\$1,arrival_flag_remark=\$2/);
      assert.match(sql,/WHERE reference=\$3 AND acceptance_required=TRUE AND accepted_at IS NULL/);
      assert.match(sql,/status NOT IN \('Closed','Idle','Ideal'\) AND started_at<=NOW\(\)-INTERVAL '1 hour'/);
      assert.match(sql,/arrival_flagged_at IS NULL AND site=\$4/);
      if(beforeUpdate)beforeUpdate(saved);
      const [actor,remark,reference,site]=values;
      if(!saved||saved.ref!==reference||!saved.acceptanceRequired||saved.acceptedAt||['Closed','Idle','Ideal'].includes(saved.status)||Date.parse(saved.start)>now-3600000||saved.arrivalFlaggedAt||saved.site!==site)return {rows:[]};
      saved={...saved,arrivalFlaggedAt:'2026-09-08 13:00:00',arrivalFlaggedBy:actor,arrivalFlagRemark:remark};
...
test('only a waiting unreceived request at least one hour old can be flagged',async()=>{
  for(const row of [{...eligible,acceptanceRequired:false},{...eligible,acceptedAt:'2026-09-08 12:00:00'},...['Closed','Idle','Ideal'].map(status=>({...eligible,status})),{...eligible,start:'2026-09-08T07:00:01Z'}]){
...
test('duplicate and concurrent arrival flags preserve the original remark and evidence',async()=>{
  ...
  const legacy=harness({row:{...eligible,arrivalFlaggedAt:'2026-09-08 12:00:00',arrivalFlaggedBy:'Original inspector'}});
  await legacy.call({remark:'Do not backfill'});
  assert.equal(legacy.saved.arrivalFlagRemark,'');
...
test('acceptance, closure and reassignment between read and write prevent a flag',async()=>{
  for(const beforeUpdate of [row=>{row.acceptedAt='2026-09-08 12:59:59';},row=>{row.status='Closed';},row=>{row.site='Majri OB';}]){
