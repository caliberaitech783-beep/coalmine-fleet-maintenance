...
test("Maintenance places Actions before Job reference without reordering other roles", () => {
  ...
  assert.match(table, /showActions && actionsFirst && <th>Actions<\/th>}\s*\{showAcceptedTime && workflowHeader\("acceptedTime", "Accepted time"\)}\s*\{workflowHeader\("ref", "Job reference"\)/);
  assert.match(table, /\{actionsFirst && workflowActions\(row, lockedIdeal\)}\s*\{showAcceptedTime && <td><b>\{elapsedLabel\(row\.start, row\.acceptedAt\)\}<\/b><\/td>}\s*<td><b>\{row\.ref\}/);
...
test("maintenance close table shows acceptance duration before job reference", () => {
  const source = fs.readFileSync(new URL("../src/main.jsx", import.meta.url), "utf8");
  assert.match(source, /isMaintenance && tab === "close"[^\n]*showAcceptedTime/);
  assert.match(source, /showAcceptedTime && workflowHeader\("acceptedTime", "Accepted time"\)[\s\S]*workflowHeader\("ref", "Job reference"\)/);
