...
test('attachment endpoint checks ownership and site before loading private media',async()=>{
 ...
 const handler=new AsyncFunction('req','res','next','pool','currentDashboardAuthorization','reportScopeIncludesSite','managerReportScope','canonicalSiteName','requestProjection','requestTimelineProjection',body);
 for(const [site,owner,expected] of [['A','alice',200],['B','alice',403],['A','bob',403]]){
  ...
  await handler(req,res,error=>{throw error;},pool,async()=>({session,user:{site:'A',login:'alice'}}),()=>false,()=>({}),x=>x,'projection','timeline');
