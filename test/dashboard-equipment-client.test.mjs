...
test("fleet panels distinguish loading and failure from a confirmed empty fleet",()=>{
  ...
  assert.match(dashboard,/equipmentLoaded \? \(mode === "oem" \? oemChart\.rows\.length : mode === "total" \? \(showOemBreakdowns \? oemFleetEquipment\.length : assetCounts\.total\) : \(showOemBreakdowns \? oemChart\.rows\.length : accountBreakdownCount\)\)\.toLocaleString\(\) : "—"/);
...
test("a failed profile request cannot turn an otherwise valid fleet response into zero",()=>{
  ...
  assert.match(normal,/<Dashboard requests=\{misDashboardRequests\} requestsError=\{dashboardRequestsError\} requestsUpdatedAt=\{dashboardRequestsUpdatedAt\}/);
