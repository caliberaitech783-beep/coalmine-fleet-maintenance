...
test("fleet panels distinguish loading and failure from a confirmed empty fleet",()=>{
  ...
  assert.match(dashboard,/equipmentLoaded \? \(mode === "oem" \? oemChart\.rows\.length : mode === "total" \? \(showOemBreakdowns \? oemFleetEquipment\.length : assetCounts\.total\) : \(showOemBreakdowns \? oemChart\.rows\.length : liveBreakdownAssetCount\)\)\.toLocaleString\(\) : "—"/);
