...
test('operational, manager and workspace navigation get the graphical treatment through data-nav attributes only',()=>{
  ...
  assert.match(source,/const productionFirstTripRows=closedRequests\.filter\(\(row\)=>isProductionFirstTripPending\(row\)\);/,'first trip tabs do not depend on the MIS verification queue');
