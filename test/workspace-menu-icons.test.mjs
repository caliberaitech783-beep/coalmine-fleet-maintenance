...
test('operational, manager and workspace navigation get the graphical treatment through data-nav attributes only',()=>{
  ...
  assert.match(source,/const productionFirstTripRows=requestRows\.filter\(\(row\)=>isProductionFirstTripPending\(row\)\);/,'first trip tabs use the shared yesterday-onward pending rule');
