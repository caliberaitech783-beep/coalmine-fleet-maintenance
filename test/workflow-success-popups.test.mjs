...
test("successful workflow actions show the requested popup notifications", () => {
  ...
  assert.match(styles, /\.workflow-success-popup\{position:fixed;top:50%;left:50%/);
  assert.match(styles, /transform:translate\(-50%,-50%\)/);
  assert.match(styles, /width:min\(440px,calc\(100vw - 32px\)\)/);
  assert.match(styles, /\.workflow-success-popup \.hierarchy-save-message\{[^}]*font-size:17px/);
});
